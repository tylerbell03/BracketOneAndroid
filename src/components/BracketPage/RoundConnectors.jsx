...
export function RoundConnectors({
  ...
  // Draws a bent connector (horizontal → vertical → horizontal) from sourceY to targetY.
  // If at the same height, draws a straight line instead.
  const drawBentConnector = (key, sourceY, targetY, color) => {
  ...
  if (numTeams === 13 && currentCount === 5 && nextCount === 4) {
    ...
    (numTeams === 11 || numTeams === 0) &&
    currentCount === 3 &&
    nextCount === 4
  ) {
    // 11-team bracket (single or double elim WB):
    // G1→next[0], G2→next[2] (skipping next[1] which is A4vA5), G3→next[3]
    const y0 = currentYCenters[0];
    const ny0 = nextYCenters[0];
    if (y0 != null && ny0 != null) {
      lines.push(...drawBentConnector("playin-0", y0, ny0, lineColor));
    }
    const y1 = currentYCenters[1];
    const ny2 = nextYCenters[2];
    if (y1 != null && ny2 != null) {
      lines.push(...drawBentConnector("playin-1", y1, ny2, lineColor));
    }
    const y2 = currentYCenters[2];
    const ny3 = nextYCenters[3];
    if (y2 != null && ny3 != null) {
      lines.push(...drawBentConnector("playin-2", y2, ny3, lineColor));
    }
    // next[1] (A4vA5) intentionally has no incoming connector
